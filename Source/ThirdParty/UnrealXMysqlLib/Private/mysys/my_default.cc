...
int my_search_option_files(const char *conf_file, int *argc, char ***argv,
                           ...
                           void *func_ctx, const char **default_directories_par,
  ...
  // If my defaults file is set (from a previous run), we read it
    ...
    for (dirs = default_directories_par; *dirs; dirs++) {
      if (**dirs) {
...
int my_load_defaults(const char *conf_file, const char **groups, int *argc,
                     char ***argv, MEM_ROOT *alloc,
                     const char ***default_directories_par) {
  ...
  *argv = const_cast<char **>(res);
  ...
  if (default_directories_par) *default_directories_par = dirs;
...
void init_variable_default_paths() {
  ...
  if (GetModuleFileNameA(NULL, buffer, (DWORD)sizeof(buffer))) {
...
static size_t my_get_system_windows_directory(char *buffer, size_t size) {
  ...
  func_ptr = (GET_SYSTEM_WINDOWS_DIRECTORY)GetProcAddress(
      GetModuleHandleA("kernel32.dll"), "GetSystemWindowsDirectoryA");
  ...
  */
  count = GetSystemDirectoryA(buffer, (uint)size);
...
static const char *my_get_module_parent(char *buf, size_t size) {
  char *last = NULL;
  char *end;
  if (!GetModuleFileNameA(NULL, buf, (DWORD)size)) return NULL;
  end = strend(buf);
...
static const char **init_default_directories(MEM_ROOT *alloc) {
...

  {
    ...
    if (GetWindowsDirectoryA(fname_buffer, sizeof(fname_buffer)))
      errors += add_directory(alloc, fname_buffer, dirs);
