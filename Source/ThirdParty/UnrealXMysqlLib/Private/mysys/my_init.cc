...
static void win_init_registry() {
  ...
  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\MySQL", 0, KEY_READ,
    ...
    while ((ret = RegEnumValueA(key_handle, index++, key_name, &key_name_len,
...
static bool win32_have_tcpip() {
  HKEY hTcpipRegKey;
  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, TCPIPKEY, 0, KEY_READ, &hTcpipRegKey) !=
      ERROR_SUCCESS) {
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, WINSOCK2KEY, 0, KEY_READ,
                     &hTcpipRegKey) != ERROR_SUCCESS) {
      if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, WINSOCKKEY, 0, KEY_READ,
                       &hTcpipRegKey) != ERROR_SUCCESS)
