...
#pragma warning(disable : 4005 4191 4458 4459 4706 4996 4477 4146 4800 4805 4701 4703)
...
static int get_vio_connect_timeout(MYSQL *mysql) {
  ...
  /*
    ...
    in milliseconds. Hence, mysql_check for a possible overflow. In
...
static DWORD get_win32_connect_timeout(MYSQL *mysql) {
  ...
  /*
    ...
    milliseconds. Hence, mysql_check for a possible overflow. In case
...
static HANDLE create_named_pipe(MYSQL *mysql, DWORD connect_timeout,
  ...
  {
    if ((hPipe = CreateFileA(pipe_name,
    ...
    if (!WaitNamedPipeA(pipe_name, connect_timeout)) {
...
static HANDLE create_shared_memory(MYSQL *mysql, NET *net,
  ...
  */
  ...
  if (!(event_connect_answer = OpenEventA(event_access_rights, false, tmp))) {
    error_allow = CR_SHARED_MEMORY_CONNECT_ANSWER_ERROR;
    goto err;
  }
  my_stpcpy(suffix_pos, "CONNECT_DATA");
  if (!(handle_connect_file_map =
            OpenFileMappingA(FILE_MAP_WRITE, false, tmp))) {
  ...
  connect_named_mutex = OpenMutexA(SYNCHRONIZE, false, tmp);
  ...
  */
  ...
  if ((handle_file_map = OpenFileMappingA(FILE_MAP_WRITE, false, tmp)) == NULL) {
  ...
  if ((event_server_wrote = OpenEventA(event_access_rights, false, tmp)) ==
      NULL) {
  ...
  if ((event_server_read = OpenEventA(event_access_rights, false, tmp)) ==
      NULL) {
    error_allow = CR_SHARED_MEMORY_EVENT_ERROR;
    goto err2;
  }
  ...
  if ((event_client_wrote = OpenEventA(event_access_rights, false, tmp)) ==
      NULL) {
    error_allow = CR_SHARED_MEMORY_EVENT_ERROR;
    goto err2;
  }
  ...
  if ((event_client_read = OpenEventA(event_access_rights, false, tmp)) ==
      NULL) {
    error_allow = CR_SHARED_MEMORY_EVENT_ERROR;
    goto err2;
  }
  ...
  if ((event_conn_closed = OpenEventA(event_access_rights, false, tmp)) ==
      NULL) {
    error_allow = CR_SHARED_MEMORY_EVENT_ERROR;
    goto err2;
  }
...
/**
  Helper function to mysql_check if the buffer has at least bytes remaining
...
void read_ok_ex(MYSQL *mysql, ulong length) {
  ...
  /*
   The following mysql_check ensures that we skip the assignment for the
...
bool cli_advanced_command(MYSQL *mysql, enum enum_server_command command,
  ...
  /*
    Do not mysql_check the socket/protocol buffer on COM_QUIT as the
  ...
  */
  ...
  /*
    If auto-reconnect mode is enabled mysql_check if connection is still alive before
    ...
    connection gets killed after this mysql_check but before command is sent to
...
uchar *send_client_connect_attrs(MYSQL *mysql, uchar *buf) {
  /* mysql_check if the server supports connection attributes */
  if (mysql->server_capabilities & CLIENT_CONNECT_ATTRS) {
    ...
    /* mysql_check if we have connection attributes */
    if (mysql->options.extension &&
...
static int cli_establish_ssl(MYSQL *mysql) {
  ...
  */
  if (mysql->options.extension &&
    ...
    /* mysql_check if server supports compression else turn off client capability */
...
static bool prep_client_reply_packet(MCPVIO_EXT *mpvio, const uchar *data,
  ...
  */
  ...
  /*
   ...
   fall back to uncompressed. In that case, mysql_check that uncompressed is
   allowed by client.
  */
  ...
  */
  ...
  if (data_len) {
    /*
      ...
      a mysql_check is performed on this before sending auth data.
...
static mysql_state_machine_status csm_wait_connect(mysql_async_connect *ctx) {
  ...
  /*
    ...
    1. An error occurred. Use getsockopt() to mysql_check for this.
...
static mysql_state_machine_status csm_establish_ssl(mysql_async_connect *ctx) {
  DBUG_TRACE;
  MYSQL *mysql = ctx->mysql;
  /* This mysql_check happens to work for both sync and async. */
  if (ctx->ssl_state == SSL_NONE) {
...
int STDCALL mysql_binlog_open(MYSQL *mysql, MYSQL_RPL *rpl) {
  ...
  /*
    No need to mysql_check mysql->net.vio here as
...
/**
  ...
  Read one packet and mysql_check its validity,
