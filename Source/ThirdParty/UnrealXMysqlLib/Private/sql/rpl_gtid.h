...
/**
  This macro is used to mysql_check that the given character, pointed to by the
...
/**
  ...
  1. It has additional operations to mysql_check if the read and/or write lock
     is held at the moment.
  2. It is wrapped in an object-oriented interface.

  Note that the assertions do not mysql_check whether *this* thread has
...
struct Gtid {
  ...
  */
  bool is_empty() const {
    // mysql_check that gno is not set inconsistently
    if (sidno <= 0)
      DBUG_ASSERT(gno == 0);
    else
      DBUG_ASSERT(gno > 0);
...
class Gtid_set {
  ...
  */
  ...
  /**
    ...
    @param sidno SIDNO to mysql_check for this Gtid_set.
    @param other Other Gtid_set
    @param other_sidno SIDNO to mysql_check in other.
...
#ifndef DBUG_OFF
  /**
    The number of chunks.  Used only to mysql_check some invariants when
    DBUG is on.
  */
  int n_chunks;
#endif
...
class Gtid_state {
  ...
  */
  ...
  /**
    ...
    @param gtid The Gtid to mysql_check.
...
/**
  ...
  In order to avoid acquiring lock to mysql_check a variable that is not changed
